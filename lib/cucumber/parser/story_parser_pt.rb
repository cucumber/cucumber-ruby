...
module Story
  ...
  def _nt_header
    ...
    i0, s0 = index, []
    if input.index('Relato:', index) == index
      r1 = (SyntaxNode).new(input, index...(index + 7))
      @index += 7
    else
      terminal_parse_failure('Relato:')
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_scenario_start
    ...
    if r1
      if input.index('Escenario: ', index) == index
        r2 = (SyntaxNode).new(input, index...(index + 11))
        @index += 11
      else
  ...
  module Step1
    def eval(listener, phase)
      ...
      listener.__send__(method, step_type.text_value.strip, sentence.text_value.strip, self) if listener.respond_to?(method)
    end
      ...
      if r3
        ...
        if r4
          ...
          if input.index('Entonces', index) == index
            r5 = (SyntaxNode).new(input, index...(index + 8))
            @index += 8
          else
