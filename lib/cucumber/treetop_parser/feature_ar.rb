...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("بفرض", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 8))
      @index += 8
    else
      terminal_parse_failure("بفرض")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("متى", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 6))
        @index += 6
      else
        terminal_parse_failure("متى")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("اذاً", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 8))
          @index += 8
        else
          terminal_parse_failure("اذاً")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("و", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 2))
            @index += 2
          else
            terminal_parse_failure("و")
            r4 = nil
          end
          if r4
            r0 = r4
          else
            if input.index("لكن", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 6))
              @index += 6
            else
              terminal_parse_failure("لكن")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("سيناريو", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 14))
      @index += 14
    else
      terminal_parse_failure("سيناريو")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_more_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("امثلة اكثر", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 19))
      @index += 19
    else
      terminal_parse_failure("امثلة اكثر")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_given_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("بفرض سيناريو معين", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 32))
      @index += 32
    else
      terminal_parse_failure("بفرض سيناريو معين")
      r1 = nil
    end
    s0 << r1
    if r1
