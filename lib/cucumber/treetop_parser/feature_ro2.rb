...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("Dacă", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 5))
      @index += 5
    else
      terminal_parse_failure("Dacă")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("Când", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 5))
        @index += 5
      else
        terminal_parse_failure("Când")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Atunci", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 6))
          @index += 6
        else
          terminal_parse_failure("Atunci")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("Și", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 3))
            @index += 3
          else
            terminal_parse_failure("Și")
            r4 = nil
          end
          if r4
            r0 = r4
          else
            if input.index("Dar", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 3))
              @index += 3
            else
              terminal_parse_failure("Dar")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Scenariu", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 8))
      @index += 8
    else
      terminal_parse_failure("Scenariu")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_more_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("Mai multe exemple", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 17))
      @index += 17
    else
      terminal_parse_failure("Mai multe exemple")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_given_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("ScenariuDat", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 11))
      @index += 11
    else
      terminal_parse_failure("ScenariuDat")
      r1 = nil
    end
    s0 << r1
    if r1
