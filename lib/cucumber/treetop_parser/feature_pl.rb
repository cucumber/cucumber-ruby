...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("Dane", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 4))
      @index += 4
    else
      terminal_parse_failure("Dane")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("Jeżeli", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 7))
        @index += 7
      else
        terminal_parse_failure("Jeżeli")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Wtedy", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 5))
          @index += 5
        else
          terminal_parse_failure("Wtedy")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("Oraz", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 4))
            @index += 4
          else
            terminal_parse_failure("Oraz")
            r4 = nil
          end
          if r4
            r0 = r4
          else
            if input.index("Ale", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 3))
              @index += 3
            else
              terminal_parse_failure("Ale")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Scenariusz", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 10))
      @index += 10
    else
      terminal_parse_failure("Scenariusz")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_more_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("WięcejPrzykładów", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 19))
      @index += 19
    else
      terminal_parse_failure("WięcejPrzykładów")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_given_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("DanyScenariusz", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 14))
      @index += 14
    else
      terminal_parse_failure("DanyScenariusz")
      r1 = nil
    end
    s0 << r1
    if r1
