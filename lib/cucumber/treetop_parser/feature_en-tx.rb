...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("Given y'all", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 11))
      @index += 11
    else
      terminal_parse_failure("Given y'all")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("When y'all", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 10))
        @index += 10
      else
        terminal_parse_failure("When y'all")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Then y'all", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 10))
          @index += 10
        else
          terminal_parse_failure("Then y'all")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("And y'all", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 9))
            @index += 9
          else
            terminal_parse_failure("And y'all")
            r4 = nil
          end
          if r4
            r0 = r4
          else
            if input.index("But y'all", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 9))
              @index += 9
            else
              terminal_parse_failure("But y'all")
              r5 = nil
            end
            if r5
              r0 = r5
