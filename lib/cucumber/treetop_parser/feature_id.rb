...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("Dengan", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 6))
      @index += 6
    else
      terminal_parse_failure("Dengan")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("Ketika", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 6))
        @index += 6
      else
        terminal_parse_failure("Ketika")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Maka", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 4))
          @index += 4
        else
          terminal_parse_failure("Maka")
          r3 = nil
        end
        if r3
          r0 = r3
          ...
          if r4
            ...
            if input.index("Tapi", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 4))
              @index += 4
            else
              terminal_parse_failure("Tapi")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Skenario", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 8))
      @index += 8
    else
      terminal_parse_failure("Skenario")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_more_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("Contoh Tambahan", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 15))
      @index += 15
    else
      terminal_parse_failure("Contoh Tambahan")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_given_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("DenganSkenario", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 14))
      @index += 14
    else
      terminal_parse_failure("DenganSkenario")
      r1 = nil
    end
    s0 << r1
    if r1
