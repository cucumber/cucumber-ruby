...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("anrhegedig a", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 12))
      @index += 12
    else
      terminal_parse_failure("anrhegedig a")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("Pryd", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 4))
        @index += 4
      else
        terminal_parse_failure("Pryd")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Yna", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 3))
          @index += 3
        else
          terminal_parse_failure("Yna")
          r3 = nil
        end
        if r3
          r0 = r3
          ...
          if r4
            ...
            if input.index("Ond", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 3))
              @index += 3
            else
              terminal_parse_failure("Ond")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_more_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("Hychwaneg enghreifftiau", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 23))
      @index += 23
    else
      terminal_parse_failure("Hychwaneg enghreifftiau")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_given_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Anrhegedig scenario", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 19))
      @index += 19
    else
      terminal_parse_failure("Anrhegedig scenario")
      r1 = nil
    end
    s0 << r1
    if r1
