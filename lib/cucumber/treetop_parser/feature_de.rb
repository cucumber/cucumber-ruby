...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index("Gegeben sei", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 11))
      @index += 11
    else
      terminal_parse_failure("Gegeben sei")
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index("Wenn", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 4))
        @index += 4
      else
        terminal_parse_failure("Wenn")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("Dann", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 4))
          @index += 4
        else
          terminal_parse_failure("Dann")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("Und", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 3))
            @index += 3
          else
            terminal_parse_failure("Und")
            r4 = nil
          end
          if r4
            r0 = r4
          else
            if input.index("Aber", index) == index
              r5 = (SyntaxNode).new(input, index...(index + 4))
              @index += 4
            else
              terminal_parse_failure("Aber")
              r5 = nil
            end
            if r5
              r0 = r5
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Szenario", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 8))
      @index += 8
    else
      terminal_parse_failure("Szenario")
      r1 = nil
    end
    s0 << r1
    if r1
