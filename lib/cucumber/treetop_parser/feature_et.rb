...
module Feature
  ...
  def _nt_step_keyword
    ...
    i0 = index
    if input.index('Eeldades', index) == index
      r1 = (SyntaxNode).new(input, index...(index + 8))
      @index += 8
    else
      terminal_parse_failure('Eeldades')
      r1 = nil
    end
    if r1
      r0 = r1
    else
      if input.index('Kui', index) == index
        r2 = (SyntaxNode).new(input, index...(index + 3))
        @index += 3
      else
        terminal_parse_failure('Kui')
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index('Siis', index) == index
          r3 = (SyntaxNode).new(input, index...(index + 4))
          @index += 4
        else
          terminal_parse_failure('Siis')
          r3 = nil
        end
        if r3
          r0 = r3
  ...
  def _nt_scenario_keyword
    ...
    if input.index('Stsenaarium', index) == index
      r0 = (SyntaxNode).new(input, index...(index + 11))
      @index += 11
    else
      terminal_parse_failure('Stsenaarium')
      r0 = nil
    end
  ...
  def _nt_given_scenario_keyword
    ...
    if input.index('EeldadesStsenaariumit', index) == index
      r0 = (SyntaxNode).new(input, index...(index + 21))
      @index += 21
    else
      terminal_parse_failure('EeldadesStsenaariumit')
      r0 = nil
    end
