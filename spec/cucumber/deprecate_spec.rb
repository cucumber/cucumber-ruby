...
module Cucumber::Deprecate
  describe 'Cucumber.deprecate' do
    it 'fails when running the tests' do
      expect { Cucumber.deprecate('Use some_method instead', 'someMethod', '1.0.0') }
        .to raise_exception('This method is due for removal after version 1.0.0')
    end
    ...
    it 'outputs a message to STDERR' do
      ...
      expect($stderr).to have_received(:puts).with(
        a_string_including('WARNING: #someMethod is deprecated and will be removed after version 1.0.0. Use some_method instead.')
      )
    end
  end
