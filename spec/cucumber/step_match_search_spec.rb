...
RSpec.describe Cucumber::StepMatchSearch do
  ...
  describe 'resolving step definition matches' do
    ...
    it 'raises Ambiguous error with guess hint when multiple step definitions match' do
      ...
      expect { search.call('Three blind mice').first }.to raise_error(Cucumber::Ambiguous, /#{elongated_error_message}/)
    end
    ...
    describe 'when --guess is used' do
      ...
      it 'does not show --guess hint' do
        ...
        expect { search.call('Three cute mice').first }.to raise_error(Cucumber::Ambiguous, /#{elongated_error_message}/)
      end
      ...
      it 'picks most specific step definition when an unequal number of capture groups' do
        _general      = dsl.Given(/Three (.*) mice ran (.*)/) {}
