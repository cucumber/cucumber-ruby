...
RSpec.describe Cucumber::Cli::ProfileLoader do
  ...
  context 'when on a Windows OS' do
    ...
    it 'treats backslashes as literals in rerun.txt when on Windows (JRuby or MRI)' do
      ...
      expect(loader.args_from('default')).to eq(%w[--format pretty features\sync_imap_mailbox.feature:16:22])
    end
    ...
    it 'treats forward slashes as literals' do
      ...
      expect(loader.args_from('default')).to eq(%w[--format ugly features/sync_imap_mailbox.feature:16:22])
    end
